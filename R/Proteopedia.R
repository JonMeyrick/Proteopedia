...
Calculate_AUC <- function(InputDirectory){
  ...
  for(FileIndex in list.files(pattern = "mzML")){
  TIC_data <- mzR::tic(mzR::openMSfile(paste0(InputDirectory, "/",FileIndex)))
  AUCs = rbind(AUCs, data.table::data.table(mzML = FileIndex, AUC = DescTools::AUC(TIC_data$rtime, TIC_data$intensity)))}
...
#' @export
Process_LF_DIANN <- function(InputDirectory, CtlGroup, ProteotypicFiltering = F, DIANNVersion = 2.2){
  start.time <- Sys.time()
  set.seed(123)
  if(DIANNVersion >= 2.2){
    ...
    message("Defining Metadata")
    {
      ...
      Metadata <- unique(PrecursorData[, .(Sample, Cell, Conc, Drug, Time, Replicate, Condition)])

      # Order Samples
      Sample_Order <- Metadata |> dplyr::arrange(!grepl(CtlGroup, Condition), Condition, Replicate)
    ...
    message("Compiling & Exporting Data")
    {
      ...
      ProteinAnnotations <- data.table::fread("report.protein_description.tsv.gz")[, .(Protein.Id, Gene)] |> data.table::setnames("Protein.Id", "ProteinGroup")
      ProteinData <- Reduce(Merge_PrecursorData, list(LFQ, tot_intensities, PrecursorCounts, proteotypic_counts)) |>
        data.table::merge.data.table(ProteinAnnotations, by = "ProteinGroup") |> data.table::merge.data.table(Metadata, by = "Sample")
    ...
    message("Exporting QC Plots")
    {
      ...
      print(Intensity_Plot + Count_Plot + patchwork::free(NAs_Plot, type = "label") +
    ...
    message("Compiling & Exporting Data")
    {
      ...
      PrecursorCounts <- PrecursorData[, .(N_precursors = data.table::uniqueN(Precursor.Id)), .(ProteinGroup, Sample)]
      proteotypic_counts <- PrecursorData[, .(N_precursors_proteotypic = sum(Proteotypic)), .(ProteinGroup, Sample)]
      ProteinAnnotations <- unique(PrecursorData[, .(ProteinGroup, Sample, Condition, Replicate, ProteinDescription, Gene)])
      ProteinData <- Reduce(Merge_PrecursorData, list(LFQ, tot_intensities, PrecursorCounts, proteotypic_counts, ProteinAnnotations))
      data.table::fwrite(ProteinData, "LF_DIANN_Output.csv.gz")
    }
    message("Plotting Intensities")
    {
    ...
    for(CtlGroup in CtlGroups){
      message("Loading Input File")
      {
        ...
        SpectraRead |> data.table::setnames(
          ...
          c("ProteinGroup", "Gene"))
        meta <- SpectraRead[grepl(ExpGroup, Condition)|grepl(CtlGroup, Condition), .(Sample, Condition, Replicate)] |> dplyr::distinct() |>
          dplyr::arrange(c(which(grepl(CtlGroup, Condition)), which(grepl(ExpGroup, Condition))))
        ProteinInfo <- SpectraRead[, .(ProteinGroup, Gene)] |> dplyr::distinct()

        SpectraRead <- SpectraRead[Sample %in% meta$Sample]
      ...
      {
        prePCA_data <- SpectraRead[, .(ProteinGroup, Gene, Sample, Log2LFQ)] |>
      ...
      {
        filtering_data <- SpectraRead[,.(N_Samples = .N, Min_Precursors = min(N_precursors), N_conditions = data.table::uniqueN(Condition)), ProteinGroup]

        filtering_data[, UniProtID := gsub("[;-].*", "", ProteinGroup)]

        Retained_1 <- filtering_data[N_Samples == data.table::uniqueN(SpectraRead$Sample) & Min_Precursors >= MinPrecursors]
        Retained_2 <- filtering_data[N_Samples == data.table::uniqueN(SpectraRead$Sample)-1 & Min_Precursors >= (MinPrecursors+1)]
        Retained_3 <- filtering_data[N_Samples == floor(data.table::uniqueN(SpectraRead$Sample)/2) & Min_Precursors >= (MinPrecursors+1) & N_conditions >= 1]

        Retained_Proteins <- c(Retained_1$ProteinGroup, Retained_2$ProteinGroup, Retained_3$ProteinGroup)
        Filtered_Proteins <- SpectraRead[ProteinGroup %!in% Retained_Proteins]
        SpectraRead <- SpectraRead[ProteinGroup %in% Retained_Proteins]
        ...
        Filtering_Plot_Data_A <- SpectraRead[,.(ProteinGroup, Sample, Condition, Replicate, Inclusion = "Retained")]
        ...
        Upset_Plot <- SpectraRead[, .(Sample = list(Sample)), by = ProteinGroup] |> ggplot2::ggplot(ggplot2::aes(x = Sample)) + ggplot2::geom_bar() +
          ggplot2::geom_text(stat="count", ggplot2::aes(label = ggplot2::after_stat(count)), vjust = -0.5, size = 3) +
          ggupset::scale_x_upset(order_by = "degree", reverse = TRUE, sets = SpectraRead[order(Condition), unique(Sample)]) +
          ggplot2::labs(x = NULL, y = stringr::str_wrap("Post-Filtering Count", 10)) +
          ggplot2::scale_y_continuous(expand = ggplot2::expansion(mult = c(0,0.15)))
      }
      message("Performing Median Normalisation")
      {
        spectra_mean <- median(SpectraRead$Log2LFQ, na.rm = TRUE)
        SpectraRead[, Log2LFQ_Norm := Log2LFQ - median(Log2LFQ, na.rm = TRUE) + spectra_mean, by = Sample]
        SpectraRead[, Colour := data.table::fifelse(Condition == CtlGroup, RColorBrewer::brewer.pal(3, "Set1")[2], RColorBrewer::brewer.pal(3, "Set1")[1])]

        Normalisation_Plot <- ggplot2::ggplot(data.table::melt.data.table(SpectraRead, measure.vars = c("Log2LFQ", "Log2LFQ_Norm")),
      ...
      {
        SpectraRead <- SpectraRead[,.(ProteinGroup, Gene, Sample, Log2LFQ_Norm)] |>
          tidyr::pivot_wider(id_cols = ProteinGroup, values_from = Log2LFQ_Norm, names_from = Sample, values_fill = NA) |>
          data.table::data.table()

        spectra_imp <- data.table::data.table(imputeLCMD::impute.MinProb(as.matrix(SpectraRead, rownames = "ProteinGroup"), q = ImputationQ, tune.sigma = ImputationSigma),
                                              keep.rownames = "ProteinGroup")
        spectra_all <- data.table::merge.data.table(data.table::melt.data.table(SpectraRead, id.vars = "ProteinGroup", value.name = "Measured_LFQ", variable.name = "Sample"),
        ...
        imputedNAs <- data.table::melt.data.table(SpectraRead[ProteinGroup %in% Retained_3$ProteinGroup], id.vars = "ProteinGroup", variable.name = "Sample")
      ...
      message("Performing Paired T-Testing")
      {
        ...
        spectra_Ttest <- spectra_Ttest |> data.table::merge.data.table(ProteinInfo) |> data.table()
      ...
      message("Fitting to Linear Model & Exporting Data")
      {
        ...
        LimmaInput <- spectra_all |> data.table::dcast(formula = ProteinGroup ~ Sample, value.var = "Log2LFQ") |>
        ...
        LimmaOutput <- LimmaOutput[order(abs(LimmaOutput$Log2FC), decreasing = TRUE),] |> data.table::merge.data.table(ProteinInfo, all.x = T)
...
#' @export
Process_StaticSILAC_DIANN <- function(InputDirectory, CtlGroup, ProteotypicFiltering = F, DIANNVersion = 2.2){
  set.seed(123)
  start.time <- Sys.time()
  if(DIANNVersion >= 2.2){
    message("Importing DIA-NN Report File")
    {
      ...
      if(nrow(PrecursorData[Channel == "Heavy"]) == 0){
    ...
    {
      SILACRatios <- data.table::dcast(PrecursorData, Precursor.Id + ProteinGroup + Sample ~ Channel, value.var = "Precursor.Normalised")
      SILACRatios[, Log2HLRatio := log2(Heavy/Light)]
    ...
    {
      ProteinAnnotations <- unique(PrecursorData[, .(ProteinGroup, Run, Sample, Condition, Cell, Drug, Conc, Time, Replicate)])
    ...
    message("Plotting Intensities")
    {
      IntensitiesData <- data.table::rbindlist(list(
        PrecursorData[, .(Sample, Condition, Replicate, Channel, `Log2 Quantity` = log2(Precursor.Normalised), Type = "Precursor Quantity")],
        ProteinData[, .(Sample, Condition, Replicate, `Log2 Quantity` = log2(LFQ_L), Channel = "Light", Type = "Max. Protein LFQ")],
        ProteinData[, .(Sample, Condition, Replicate, `Log2 Quantity` = log2(Intensity_L), Channel = "Light", Type = "Protein Intensity")],
        ProteinData[, .(Sample, Condition, Replicate, `Log2 Quantity` = log2(LFQ_H), Channel = "Heavy", Type = "Max. Protein LFQ")],
        ProteinData[, .(Sample, Condition, Replicate, `Log2 Quantity` = log2(Intensity_H), Channel = "Heavy", Type = "Protein Intensity")],
        ProteinData[, .(Sample, Condition, Replicate, `Log2 Quantity` = log2(LFQ_T), Channel = "Total", Type = "Max. Protein LFQ")],
        ProteinData[, .(Sample, Condition, Replicate, `Log2 Quantity` = log2(Intensity), Channel = "Total", Type = "Protein Intensity")]
      ), use.names = TRUE)
      IntensitiesData[, Type := factor(Type, levels = c("Precursor Quantity", "Max. Protein LFQ", "Protein Intensity"))]

      IntensityPlot <- IntensitiesData |> ggplot2::ggplot(ggplot2::aes(x = forcats::fct_rev(gsub("_", " ", Sample)), y = `Log2 Quantity`, colour = Condition, alpha = Channel)) +
        ...
        ggplot2::facet_grid(cols = ggplot2::vars(Type), rows = ggplot2::vars(Channel), scales = "free_x") +
    ...
    message("Plotting Isotope Incorporation")
    {
      ...
      IsotopeIncorporation <- FullIsotopeRatio |> ggplot2::ggplot(ggplot2::aes(x = stringr::str_wrap(gsub("_", " ", Sample), 5), y = Prop, fill = Condition, alpha = Channel)) +
        ...
        ggplot2::scale_y_continuous(expand = c(0,0)) + ggplot2::theme(axis.title.x = ggplot2::element_blank())
    }
    message("Plotting Precursor, Peptide & Protein Counts")
    {
      CountPlot <- data.table::melt(PrecursorData[!is.na(Channel), lapply(.SD, data.table::uniqueN), .(Sample, Condition, Channel), .SDcols = c("Precursor.Id", "Stripped.Sequence", "ProteinGroup")],
        ...
        ggplot2::ggplot(ggplot2::aes(x = forcats::fct_rev(gsub("_", " ", Sample)), y = IDs/1000, fill = Condition, alpha = Channel,
        ...
        ggplot2::facet_grid(ggplot2::vars(Channel), ggplot2::vars(variable), scales = "free_x",
    ...
    message("Plotting Missed Trypsinisation Sites")
    {
      ...
      TrypsinData[, N_Trypsin := .N, by = .(Sample, MissedTrypsin, Channel)]
      TrypsinData[, N_Sample := .N, by = .(Sample, Channel)]
      TrypsinData <- TrypsinData[MissedTrypsin == T, .(Sample, Condition, Replicate, Channel, N_Trypsin, N_Sample)] |> dplyr::distinct()
      TrypsinData[, PercentTrypsin := (N_Trypsin/N_Sample)*100]

      MissedCleavagePlot <- TrypsinData |> ggplot2::ggplot(ggplot2::aes(x = forcats::fct_rev(gsub("_", " ", Sample)), y = PercentTrypsin, fill = Condition, alpha = Channel)) +
        ggplot2::geom_bar(stat = "identity", position = "stack") + ggplot2::scale_fill_manual(values = Proteopedia::NiceColourPalette, guide = "none") +
        ggplot2::labs(y = "Precursors with Missed\nTryptic Sites (%)") + ggplot2::scale_alpha_manual(values = c("Heavy" = 0.5, "Light" = 1), guide = "none") +
        ggplot2::scale_y_continuous(expand = c(0, 0)) + ggplot2::coord_flip() +
        ggplot2::theme(axis.title.y = ggplot2::element_blank(), strip.background = ggplot2::element_blank(),
                                  strip.text.y = ggplot2::element_text(size = 26))
    }
    message("Calculating Precursor & Protein Variation")
    {
      ...
      VariationPlot <- AllCVs |> ggplot2::ggplot(ggplot2::aes(x = rank/1000, y = CV, colour = gsub("_", " ", Condition), alpha = Channel)) +
    ...
    {
      pdf("PrecursorQC_DIANN_Plot.pdf", width = 18, height = 20)
      print(IntensityPlot + CountPlot + patchwork::free(NAsPlot) + patchwork::free(MissedCleavagePlot) + IsotopeIncorporation + patchwork::free(VariationPlot) +
              patchwork::plot_layout(design = "AAAAAA\nBBBBBB\nCCDDEE\nFFFFEE") + patchwork::plot_annotation(tag_levels = "A"))
      Proteopedia::Reset_Dev()
    }
  } else {
    message("Importing DIA-NN Report File")
    ...
    message("Compiling & Exporting Data")
    {
      ...
      LFQ_T <- Proteopedia::Calculate_LFQ(PrecursorData, "LFQ")
      ...
      LFQ_L <- Proteopedia::Calculate_LFQ(PrecursorData[Label == "L"], "LFQ_L")
      ...
      LFQ_H <- Proteopedia::Calculate_LFQ(PrecursorData[Label == "H"], "LFQ_H")
      ...
      ProteinAnnotations <- unique(PrecursorData[, .(ProteinGroup, Run, Sample, Condition, Cell, Drug, Conc, Time, Replicate, ProteinDescription, Gene)])
      ProteinData <- Reduce(Proteopedia::Merge_PrecursorData, list(LFQ_T, LFQ_H, LFQ_L, Intensity_T, Intensity_H, Intensity_L, Counts_T, Counts_H, Counts_L, ProteinAnnotations))
...
#' @export
Analyse_StaticSILAC_Proteins <- function(InputDirectory, ExpGroup, CtlGroup, MinPrecursors = 2, ImputationQ = 0.01, ImputationSigma = 1, DIANNVersion = 2.2){
  set.seed(123)
  start.time <- Sys.time()
  if(DIANNVersion >= 2.2){
    message("Loading & Formatting Data")
    {
      ...
      if(length(list.files(pattern = "SILAC_DIANN_Output.csv")) > 0){
        ...
        ProteinInfo <- data.table::fread("report.protein_description.tsv.gz") |> data.table::setnames("Protein.Id", "ProteinGroup")
        ProteinInfo[, `:=`(GeneGroup, sapply(strsplit(ProteinGroup, ";"), function(x){paste(unique(gsub("-.+", "", x)), collapse = ";")}))]

        SpectraRead <- data.table::fread(InputFile)[, Log2LFQ_H := log2(LFQ_H)][, Log2LFQ_L := log2(LFQ_L)][, Log2LFQ := log2(LFQ_T)][Sample %in% meta$Sample]
    ...
    message("Analysing Both Channels As Label-Free Proxy")
    {
      ...
      {
        SpectraReadT <- data.table::data.table(tidyr::pivot_wider(SpectraReadT[, .(ProteinGroup, Sample, Log2LFQ_Norm)],
    ...
    message("Analysing Each Channel For Stability/Synthesis Measures")
    {
      ...
      message("SILAC Channels: Filtering Proteins")
      {
        FilteringData <- SpectraRead |> dplyr::group_by(ProteinGroup) |> dplyr::summarise(N_Samples = dplyr::n(),
                                                                                          ...
                                                                                          N_conditions = data.table::uniqueN(Condition)) |> data.table::data.table()
        ...
        Retained3_L <- FilteringData[N_Samples == floor(data.table::uniqueN(SpectraRead_L$Sample)/2) & Min_Precursors_L >= (MinPrecursors+1) & N_conditions == 1]
        ...
        Retained3_H <- FilteringData[N_Samples == floor(data.table::uniqueN(SpectraRead_H$Sample)/2) & Min_Precursors_H >= (MinPrecursors+1) & N_conditions == 1]
      ...
      message("SILAC Channels: Performing Median Normalisation")
      {
        ...
        NormalisationPlot <- ggplot2::ggplot(data.table::melt.data.table(SpectraRead_L |> rbind(SpectraRead_H), measure.vars = c("Log2LFQ", "Log2LFQ_Norm")),
      ...
      {
        LFQComparison <- SpectraAll |> data.table::copy() |> data.table::merge.data.table(meta)
        LFQComparison <- LFQComparison[, .(MeanLog2LFQ = mean(Log2LFQ)), .(Condition, Dataset, ProteinGroup)] |>
          data.table::dcast(ProteinGroup+Dataset ~ Condition, value.var = "MeanLog2LFQ")

        LFQComparison |> data.table::setnames(c(colnames(LFQComparison)[grepl(CtlGroup, colnames(LFQComparison))],
                                 colnames(LFQComparison)[grepl(ExpGroup, colnames(LFQComparison))]),
                               c("CtlGroup", "ExpGroup"), skip_absent = T)
        LFQComparison <- LFQComparison |> data.table::merge.data.table(Proteopedia, all.x = T)
        LFQComparison[, `:=`(GeneGroup, sapply(strsplit(ProteinGroup, ";"), function(x){paste(unique(gsub("-.+", "", x)), collapse = ";")}))]
        LFQComparison[, Histone := data.table::fifelse(grepl("Histone", ProteinDescription), T, F)]


        LFQComparison |> ggplot2::ggplot(ggplot2::aes(x = CtlGroup, y = ExpGroup)) + ggplot2::geom_point(stroke = NA) +
          ggplot2::labs(x = expression("Log"[2]~"LFQ"), y = expression("Log"[2]~"LFQ"), title = paste0(ExpGroup, " vs. ", CtlGroup)) +
          ggplot2::facet_wrap(~Dataset) + Proteopedia::Add_Rsq() + Proteopedia::Add_XYLine()

        pdf(paste0(ExpGroup, "_", CtlGroup, "_LFQComp.pdf"), height = 10, width = 14)
        print(LFQComparison |> ggplot2::ggplot(ggplot2::aes(x = CtlGroup, y = ExpGroup)) + ggplot2::geom_point(stroke = NA) +
                Proteopedia::Add_XYLine("grey") + Proteopedia::Add_Rsq(Subgroups = F) + ggplot2::facet_wrap(~Dataset) +
                ggplot2::annotate("text", x = max(LFQComparison$CtlGroup, na.rm = T)*0.9, y = min(LFQComparison$ExpGroup, na.rm = T)*0.9, label = "Histones", colour = "#F0F") +
      ...
      message("SILAC Channels: Performing Paired T-Testing")
      {
        ...
        # Light T-Test
        {
          ...
          SpectraTtest_L <- data.table::merge.data.table(data.table::data.table(SpectraTtest_L), Ttest_Output, by = "ProteinGroup")
        ...
        # Heavy T-Test
        {
          ...
          SpectraTtest_H <- data.table::merge.data.table(data.table::data.table(SpectraTtest_H), Ttest_Output, by = "ProteinGroup")
      ...
      {
        LimmaInput_L <- SpectraAll[Dataset == "Light"] |> data.table::copy() |> data.table::dcast(ProteinGroup+Dataset ~ Sample, value.var = "Log2LFQ") |> data.table::setcolorder(c("ProteinGroup", rownames(design)))
        LimmaInput_H <- SpectraAll[Dataset == "Heavy"] |> data.table::copy() |> data.table::dcast(ProteinGroup+Dataset ~ Sample, value.var = "Log2LFQ") |> data.table::setcolorder(c("ProteinGroup", rownames(design)))
        ...
        data.table::fwrite(LimmaOutput_L[, .(ProteinGroup, Isoforms, Gene, Description, Log2FC, P.Value, AveExpr, t, adj.P.Val, B, Significance, Imputed, GeneGroup)][, Dataset := "Light"] |>
                            rbind(LimmaOutput_H[, .(ProteinGroup, Isoforms, Gene, Description, Log2FC, P.Value, AveExpr, t, adj.P.Val, B, Significance, Imputed, GeneGroup)][, Dataset := "Heavy"]), file = "Limma_Output.csv")
      }
      message("SILAC Channels: Generating Volcano Plots")
      {
        ...
        InteractiveData_L <- data.table::data.table(Protein = LimmaOutput_L[, Gene],
        ...
        InteractiveData_H <- data.table::data.table(Protein = LimmaOutput_H[, Gene],
    ...
    message("Analysing Channel Ratio For Turnover Measures")
    {
      ...
      {
        PCAData <- stats::prcomp(t(data.frame(tidyr::drop_na(data.table::dcast(RatioData, ProteinGroup ~ Sample, value.var = "Log2HLRatio",
      ...
      {
        RatioDataWide <- RatioData[, .(Log2MeanRatio = mean(Log2HLRatio)), by = list(ProteinGroup, Condition)] |>
          data.table::dcast(ProteinGroup ~ Condition, value.var = "Log2MeanRatio") |>
          data.table::merge.data.table(ProteinInfo)
        RatioDataWide |> data.table::setnames(c(colnames(RatioDataWide)[grepl(CtlGroup, colnames(RatioDataWide))],
                                              colnames(RatioDataWide)[grepl(ExpGroup, colnames(RatioDataWide))]), c("CtlGroup", "ExpGroup"))
        RatioDataWide[, `:=`(GeneGroup, sapply(strsplit(ProteinGroup, ";"), function(x){paste(unique(gsub("-.+", "", x)), collapse = ";")}))]
        RatioDataWide[, Histone := data.table::fifelse(grepl("Histone", Description), T, F)]

        RatioDataLong <- RatioDataWide |> data.table::melt.data.table(id.vars = c("ProteinGroup", "Description", "Gene"),
        ...
        print(RatioDataWide |> ggplot2::ggplot(ggplot2::aes(x = CtlGroup, y = ExpGroup)) + ggplot2::geom_point(stroke = NA) +
                ...
                ggplot2::labs(x = expression("Log"[2]~"Heavy:Light Protein LFQ Ratio"), y = expression("Log"[2]~"Heavy:Light Protein LFQ Ratio"),
        ...
        InteractiveData <- data.table::data.table(Protein = RatioDataWide$Gene,
        ...
        pHC <- highcharter::hc_colors(highcharter::hc_plotOptions(highcharter::hc_tooltip(highcharter::hc_yAxis(
          ...
          scatter = list(marker = list(radius = 3), states = list(hover = list(enabled = TRUE), inactive = list(enabled = FALSE)),
                        point = list(events = list(click = htmlwidgets::JS("function() { window.open(this.URL, '_blank'); }"))))),
          c("#999999", "#880000", "#0033FF")) |> highcharter::hc_title(text = paste0(ExpGroup, " vs. ", CtlGroup))
        htmlwidgets::saveWidget(pHC, "InteractiveCorrPlot.html")
      }
      message("Channel Ratio: Performing Median Normalisation")
      {
        RatioData[, `:=`(LFQ_Ratio_Norm, Log2HLRatio - median(Log2HLRatio, na.rm = T) + median(RatioData$Log2HLRatio, na.rm = T)), by = Sample]
        NormalisationPlot <- ggplot2::ggplot(data.table::melt.data.table(RatioData, measure.vars = c("Log2HLRatio", "LFQ_Ratio_Norm")),
                                            ggplot2::aes(x = Condition, colour = Condition, y = value, group = Sample)) +
      ...
      {
        RatioData <- data.table::data.table(tidyr::pivot_wider(RatioData[, .(ProteinGroup, Sample, LFQ_Ratio_Norm)],
      ...
      message("Channel Ratio: Performing Paired T-Testing")
      {
        ...
        Ratio_Ttest <- tidyr::pivot_wider(dplyr::summarise(dplyr::group_by(RatioAll, Condition, ProteinGroup), N = dplyr::n(),
                                                          Log2MeanRatio = mean(Ratio), CV = (sd(Ratio)/mean(Ratio)) * 100),
        ...
        Ratio_Ttest <- data.table::merge.data.table(data.table::data.table(Ratio_Ttest), Ttest_Output, by = "ProteinGroup")
      ...
      message("Channel Ratio: Generating Volcano Plots")
      {
        ...
        InteractiveData <- data.table::data.table(Protein = LimmaOutput[, Gene],
                                      Log2FC = round(LimmaOutput[, Log2FC], digits = 2),
                                      PValue = LimmaOutput[, P.Value],
                                      Significance = factor(LimmaOutput[, Significance]),
                                      Scien_PValue = formatC(LimmaOutput[, P.Value], format = "e", digits = 2),
...
#' @export
Process_TimecourseSILAC_DIANN <- function(InputDirectory, ProteotypicFiltering = F, DIANNVersion = 2.2){
  set.seed(123)
  start.time <- Sys.time()
  setwd(InputDirectory)
  if(DIANNVersion >= 2.2){
    if(length(list.files(pattern = "AUC_Data.csv")) != 1){
      message("No AUC_Data.csv File Available\nUsing Summed Quantity Normalisation")
    }
  }
  message("Loading Precursor Data & Generating Metadata")
  {
    if(length(list.files(pattern = "report.parquet")) > 0){
      PrecursorData <- data.table::data.table(arrow::read_parquet(list.files(pattern = "report.parquet")[1]))[, .(Run, Protein.Group, Protein.Ids, Stripped.Sequence, Channel,
                                                                                                                  Precursor.Id, Proteotypic, Precursor.Normalised, Q.Value,
                                                                                                                  PG.Q.Value, Lib.Q.Value, Lib.PG.Q.Value,
                                                                                                                  Global.Q.Value, Global.PG.Q.Value, Quantity.Quality,
                                                                                                                  Channel.Q.Value)] |> data.table::setnames("Protein.Group", "ProteinGroup")
    ...
    if(DIANNVersion < 2.2){
      ...
      All_PCA <- All_PCA[Precursor.Normalised == 0, Precursor.Normalised := NA][, Precursor.Id := paste0(Precursor.Id, "_", Channel)] |> tidyr::pivot_wider(id_cols = Precursor.Id, values_from = Precursor.Normalised, names_from = Sample, values_fill = NA) |>
        tidyr::drop_na() |> data.frame(row.names = "Precursor.Id") |> t() |> stats::prcomp(scale. = TRUE)
      SummaryPCA <- summary(All_PCA)$importance
      All_PCA <- data.table::data.table(All_PCA$x, keep.rownames = "Sample")[, .(Sample, PC1, PC2, PC3, PC4)] |> data.table::merge.data.table(Metadata)
  ...
  message("Filtering Precursors")
  {
    if(DIANNVersion < 2.2){
      ...
      N_Samples <- PrecursorData[, .(Original = sum(Precursor.Normalised, na.rm = T)), .(Run, Sample)] |>
    ...
    if(length(list.files(pattern = "AUC_Data.csv")) == 1){
      ...
      SummedPrecursorQuants <- PrecursorData[, .(SummedQuants = sum(Precursor.Normalised, na.rm = T)), by = Run]
    ...
    if(DIANNVersion < 2.2){
      ...
      PreIonCV_Data <- PrecursorData[, .(CV = sd(Precursor.Normalised, na.rm = T)/mean(Precursor.Normalised, na.rm =T),
                                         N_Points = .N, Avg_Quant = mean(Precursor.Normalised, na.rm = T),
                                         Quality = mean(Quantity.Quality, na.rm = T)), by = .(Precursor.Id, Channel, Condition)]
      PrecursorData <- PrecursorData[!is.na(Precursor.Normalised) & Q.Value <= 0.01 & PG.Q.Value <= 0.05 & Lib.Q.Value <= 0.01 & Lib.PG.Q.Value <= 0.01 & Quantity.Quality >= 0.2 &
                                       Channel.Q.Value <= max(c(0.01, min(PrecursorData[Channel == "Heavy" & grepl(".*_0_\\d", Sample)]$Channel.Q.Value, na.rm = T)))]
      PostIonCV_Data <- PrecursorData[, .(CV = sd(Precursor.Normalised, na.rm = T)/mean(Precursor.Normalised, na.rm =T),
                                          N_Points = .N, Avg_Quant = mean(Precursor.Normalised, na.rm = T),
                                          Quality = mean(Quantity.Quality, na.rm = T)), by = .(Precursor.Id, Channel, Condition)]
    }
    ...
    All_PCA <- PrecursorData |> data.table::copy()
    if(DIANNVersion >= 2.2){
      All_PCA <- All_PCA[, Precursor.Id := paste0(Precursor.Id, "_", Channel)] |> tidyr::pivot_wider(id_cols = Precursor.Id, values_from = Precursor.Normalised, names_from = Sample, values_fill = NA) |>
        tidyr::drop_na() |> data.frame(row.names = "Precursor.Id") |> t() |> stats::prcomp(scale. = TRUE)
      SummaryPCA <- summary(All_PCA)$importance
      All_PCA <- data.table::data.table(All_PCA$x, keep.rownames = "Sample")[, .(Sample, PC1, PC2, PC3, PC4)] |> data.table::merge.data.table(Metadata)
    } else {
  ...
  message("Normalising Data")
  {
    if(length(list.files(pattern = "AUC_Data.csv")) == 1){
      ...
      if(DIANNVersion < 2.2){
        ...
        PrecursorData[, Precursor.Normalised   := Precursor.Normalised / AUC * median(AUCs$AUC), by = Sample]
        PrecursorData <- PrecursorData[!is.na(Precursor.Normalised)]
      }
    } else {
      if(DIANNVersion < 2.2){
        PrecursorData[, `:=`(Precursor.Normalised, Precursor.Quantity/sum(Precursor.Quantity, na.rm = T) * PrecursorData[, sum(Precursor.Quantity, na.rm = T), Run][, median(V1, na.rm = T)]), by = Run]
      ...
      PrecursorData[, `:=`(Precursor.Normalised, Precursor.Normalised/sum(Precursor.Normalised, na.rm = T) * PrecursorData[, sum(Precursor.Normalised, na.rm = T), Run][, median(V1, na.rm = T)]), by = Run]
      PrecursorData <- PrecursorData[!is.na(Precursor.Normalised)]
    }
    ...
    Log2Intensity_Density <- PrecursorData |> ggplot2::ggplot(ggplot2::aes(x = log2(Precursor.Normalised), fill = Cluster)) +
    ...
    if(DIANNVersion >= 2.2){
      CorrSamples <- stats::cor(CorrData[, Precursor.Id := paste0(Precursor.Id, "_", Channel)] |> data.table::dcast(Precursor.Id~Sample , value.var =  "Precursor.Normalised") |> tibble::column_to_rownames("Precursor.Id"),
    ...
    AnnotationColours <- list(Cluster = Proteopedia::NiceColourPalette[1:length(ClusterLevels)],
                              Time = Proteopedia::ThermalPalette[seq(1, length(Proteopedia::ThermalPalette), length.out = length(TimeLevels))],
    ...
    Corr_Heatmap <- pheatmap::pheatmap(CorrSamples, main = "Spearman Complete Observation Post-Filtering", show_rownames = T, show_colnames = F,
                                       annotation_col = AnnotationCol, annotation_colors = AnnotationColours, filename = "SpearmanHeatmap.pdf")
    Proteopedia::Reset_Dev()
  }
  message("Processing SILAC Labels & Calculating Ratios")
  {
    if(DIANNVersion < 2.2){
      ...
      PrecursorData[, Log2Precursor.Value := log2(Precursor.Normalised)]
    }
    PrecursorData <- PrecursorData[, Channel := data.table::fifelse(nchar(Channel) == 0, NA, Channel)]
    PrecursorData <- PrecursorData[!is.na(Channel)]

    ChannelRatio_Data <- PrecursorData |> data.table::dcast(Precursor.Id.nolabels+ProteinGroup+Sample ~ Channel, value.var = "Log2Precursor.Value")
    ...
    ChannelCounts <- PrecursorData[, .(Count = sum(Precursor.Normalised)), .(Sample, Channel)] |>
      data.table::merge.data.table(PrecursorData[, .(TotalCount = sum(Precursor.Normalised)), by = Sample]) |>
      data.table::merge.data.table(Metadata)
    ChannelCounts[, ChannelProp := Count/TotalCount]

    ChannelPropPlot <- ChannelCounts |> ggplot2::ggplot(ggplot2::aes(x = Time, y = ChannelProp, alpha = Channel, fill = Cluster)) +
  ...
  {
    ProteinWeights <- PrecursorData[,.(Sum_Intensities  = sum(log2(Precursor.Normalised), na.rm = T)), .(Sample, Channel, ProteinGroup)] |>
      data.table::merge.data.table(Metadata)
    ProteinWeights[, MeanSum := mean(Sum_Intensities, na.rm =T ), by = .(ProteinGroup, Channel)]
  ...
  message("Compiling & Exporting Data")
  {
    LFQ_T <- Calculate_LFQ(PrecursorData, "LFQ")  # For total sample (all precursors)
    LFQ_H <- Calculate_LFQ(PrecursorData[Channel == "Heavy"], "LFQ_H")  # For heavy fraction
    LFQ_L <- Calculate_LFQ(PrecursorData[Channel == "Light"], "LFQ_L")  # For light fraction
    Intensities_T <- PrecursorData[, .(Intensity = sum(Precursor.Normalised)), .(ProteinGroup, Sample)]  # For total sample (all precursors)
    Intensities_H <- PrecursorData[Channel == "Heavy", .(Intensity_H = sum(Precursor.Normalised)), .(ProteinGroup, Sample)]  # For heavy fraction
    Intensities_L <- PrecursorData[Channel == "Light", .(Intensity_L = sum(Precursor.Normalised)), .(ProteinGroup, Sample)]  # For light fraction
    Counts_H <- PrecursorData[Channel == "Heavy", .(N_precursors_H = data.table::uniqueN(Precursor.Id), N_precursors_proteotypic_H = sum(Proteotypic)), .(ProteinGroup, Sample) ]
    Counts_L <- PrecursorData[Channel == "Light", .(N_precursors_L = data.table::uniqueN(Precursor.Id), N_precursors_proteotypic_L = sum(Proteotypic)), .(ProteinGroup, Sample) ]
    if(DIANNVersion < 2.2){
      ProteinAnnotations <- unique(PrecursorData[, .(ProteinGroup, Run, Sample, Condition, Cluster, Cell, Drug, Conc, Time, Replicate, ProteinDescription, Gene)])
    } else {
      ProteinAnnotations <- data.table::fread("report.protein_description.tsv.gz")[, .(Protein.Id, Gene, Description)] |> data.table::setnames(c("Protein.Id"), c("ProteinGroup"))
    }
    ProteinData <- Reduce(Proteopedia::Merge_PrecursorData, list(LFQ_T, LFQ_H, LFQ_L, Intensities_T, Intensities_H, Intensities_L,
                                                                 Counts_H, Counts_L)) |> data.table::merge.data.table(ProteinAnnotations, by = "ProteinGroup") |>
      data.table::merge.data.table(Metadata, by = "Sample")
    ProteinData <- ProteinData[!is.na(Intensity)]
    ...
    LightLFQIntensity_Plot <- ProteinData[order(ProteinGroup)][1:1000,] |> ggplot2::ggplot(ggplot2::aes(x = LFQ_L, y = Intensity_L, colour = ProteinGroup)) +
    ...
    HeavyLFQIntensity_Plot <- ProteinData[order(ProteinGroup)][1:1000,] |> ggplot2::ggplot(ggplot2::aes(x = LFQ_H, y = Intensity_H, colour = ProteinGroup)) +
      ggplot2::geom_point(stroke = NA, alpha = 0.5) + ggplot2::labs(x = "Heavy Protein LFQ", y = "Heavy Protein Intensity") +
      ggplot2::scale_x_log10() +ggplot2::scale_y_log10() + Proteopedia::Add_XYLine("grey") + Proteopedia::Add_Rsq(F) +
      ggplot2::theme(legend.position = "none")
  ...
  {
    IntensitiesData <- data.table::rbindlist(list(
      PrecursorData[!is.na(Channel), .(Sample, Channel, `log2 quantity` = log2(Precursor.Normalised), Type = "Precursor Quantity")],
      ProteinData[, .(Sample, `log2 quantity` = log2(LFQ_L), Channel = "Light", Type = "Max. Protein LFQ")],
      ProteinData[, .(Sample, `log2 quantity` = log2(Intensity_L), Channel = "Light", Type = "Protein Intensity")],
      ProteinData[, .(Sample, `log2 quantity` = log2(LFQ_H), Channel = "Heavy", Type = "Max. Protein LFQ")],
      ProteinData[, .(Sample, `log2 quantity` = log2(Intensity_H), Channel = "Heavy", Type = "Protein Intensity")],
      ProteinData[, .(Sample, `log2 quantity` = log2(LFQ), Channel = "Total", Type = "Max. Protein LFQ")],
      ProteinData[, .(Sample, `log2 quantity` = log2(Intensity), Channel = "Total", Type = "Protein Intensity")]), use.names = TRUE) |>
      data.table::merge.data.table(Metadata)
    IntensitiesData[, Type := factor(Type, levels = c("Precursor Quantity", "Max. Protein LFQ", "Protein Intensity"))]
  ...
  message("Plotting Precursor, Peptide & Protein Counts")
  {
    CountData <- data.table::melt.data.table(PrecursorData[!is.na(Channel), lapply(.SD, data.table::uniqueN), .(Sample, Condition, Channel), .SDcols = c("Precursor.Id", "Stripped.Sequence", "ProteinGroup")],
                                             id.vars = c("Sample","Condition", "Channel"), value.name = "IDs") |> data.table::merge.data.table(Metadata)

    CountPlot <- CountData |> ggplot2::ggplot(ggplot2::aes(x = factor(gsub("_", " ", Sample), levels = gsub("_", " ", SampleLevels)), y = IDs/1000, fill = Cluster, alpha = Channel,
                                                           label = format(IDs, big.mark = ",", scientific = FALSE))) +
      ggplot2::scale_fill_manual(values = Proteopedia::NiceColourPalette, guide = "none") + ggplot2::geom_bar(stat = "identity") + ggplot2::geom_text(size = 4, hjust = 1.1) +
      ggplot2::facet_grid(ggplot2::vars(Channel), ggplot2::vars(variable), scales = "free_x",
                          labeller = ggplot2::as_labeller(c(Precursor.Id = "Precursors", Stripped.Sequence = "Peptides", ProteinGroup = "Protein Groups",
  ...
  message("Plotting Missed Trypsinisation Sites")
  {
    ...
    TrypsinData[, `:=`(N_Trypsin, .N), by = .(Sample, MissedTrypsin, Channel)]
    TrypsinData[, `:=`(N_Sample, .N), by = .(Sample, Channel)]
    TrypsinData <- dplyr::distinct(TrypsinData[MissedTrypsin == T, .(Sample, Condition, Replicate, Channel, N_Trypsin, N_Sample)]) |>
      data.table::merge.data.table(Metadata)
    TrypsinData[, `:=`(PercentTrypsin, (N_Trypsin/N_Sample) * 100)]
    MissedCleavagePlot <- ggplot2::ggplot(TrypsinData, ggplot2::aes(x = Replicate, y = PercentTrypsin, fill = Cluster, alpha = Channel)) +
      ggplot2::geom_bar(stat = "identity", position = "stack") + ggplot2::scale_fill_manual(values = Proteopedia::NiceColourPalette, guide = "none") +
  ...
  message("Calculating Precursor & Protein Variation")
  {
    PrecursorCVs <- PrecursorData[, .(CV = Proteopedia::Calculate_CV(Precursor.Normalised), N = .N), .(Precursor.Id, Condition, Cluster, Cell, Drug, Conc, Time, Channel)]
    PrecursorCVs <- PrecursorCVs[N >= 3]
    PrecursorCVs[, `:=`(rank, data.table::frank(CV)), .(Condition, Cluster, Cell, Drug, Conc, Time, Channel)]
    PrecursorCVs[, `:=`(ID, "Precursors")]
    ProteinCVs <- ProteinData[, .(CV = Proteopedia::Calculate_CV(LFQ_L), N = .N), .(ProteinGroup, Cell, Drug, Conc, Time, Condition, Cluster)][, `:=`(Channel, "Light")] |>
      rbind(ProteinData[, .(CV = Proteopedia::Calculate_CV(LFQ_H), N = .N), .(ProteinGroup, Cell, Drug, Conc, Time, Condition, Cluster)][, `:=`(Channel, "Heavy")])
    ProteinCVs <- ProteinCVs[N >= 3]
    ProteinCVs <- ProteinCVs[, `:=`(rank, data.table::frank(CV)), .(Condition, Cluster, Cell, Drug, Conc, Time, Channel)]
    ProteinCVs[, `:=`(ID, "Protein Groups")]
    AllCVs <- rbind(PrecursorCVs[, `:=`(Precursor.Id, NULL)], ProteinCVs[, `:=`(ProteinGroup, NULL)])
    VariationPlot <- AllCVs |> ggplot2::ggplot(ggplot2::aes(x = rank/1000, y = CV, colour = Cluster, alpha = Channel)) +
  ...
  message("Exporting QC Plots")
  {
    ...
    print(patchwork::free(IonCV_Plot) + patchwork::free(QuantityQuality_Density) + patchwork::free(Log2Intensity_Density) +
            patchwork::free(ChannelPropPlot) + patchwork::free(ProteinWeight_Density) + patchwork::free(LightLFQIntensity_Plot) + patchwork::free(HeavyLFQIntensity_Plot) +
            patchwork::plot_layout(design = "AABC\nDDEE\nFFGG") + patchwork::plot_annotation(tag_levels = "A"))
    #print(IntensityPlot)
    #print(CountPlot)
...
Analyse_TimecourseSILAC_Proteins <- function(InputDirectory, CtlGroup, ExpGroups, GenerateDataPlots = F, SameInitialAbundance = T, LightMinSamples = 0.5,
  ...
  for(ExpGroup in ExpGroups){
    ...
    message(paste0(ExpGroup, " vs. ", CtlGroup, ": Loading Data"))
    {
      ...
      ProtWeights |> data.table::setnames(c("Channel", colnames(ProtWeights)[grepl("Protein.*group", colnames(ProtWeights), ignore.case = T)], colnames(ProtWeights)[grepl("weight", colnames(ProtWeights), ignore.case = T)],
      ...
      ProtWeights[, Channel := gsub("L$", "Light", Channel)]
      ProtWeights[, Channel := gsub("H$", "Heavy", Channel)]
      ProtWeights <- ProtWeights[, .(Run, Sample, Channel, ProteinGroup, SumIntensities, MeanSum, Weight)] |> data.table::merge.data.table(Metadata)
    ...
    message(paste0(ExpGroup, " vs. ", CtlGroup, ": Isolating Light Channel Proteins"))
    {
      ...
      ProtWeights_L <- ProtWeights[Channel == "Light"] |> data.table::dcast(ProteinGroup ~ Sample, value.var = "Weight") |>
    ...
    message(paste0(ExpGroup, " vs. ", CtlGroup, ": Isolating Heavy Channel Proteins"))
    {
      ...
      ProteinLFQs_H <- data.table::melt.data.table(ProteinLFQs_H, id.vars = "ProteinGroup", variable.name = "Sample", value.name = "Abundance") |>
        data.table::merge.data.table(Metadata)
      ProtWeights_H <- ProtWeights[Channel =="Heavy"][,`:=`(Replicate = factor(Replicate), ProteinGroup = factor(ProteinGroup),
    ...
    if(SameInitialAbundance == F){
      ...
      {
        setwd(InputDirectory)
        SpectraRead <- data.table::fread(list.files(pattern = "ProteinQuantities"))[Time == 0][, Log2LFQ := log2(LFQ)]
        ...
        SpectraRead[, Condition := factor(Condition, levels = c(unique(SpectraRead[grepl(CtlGroup, Condition), Condition]),
                                                                unique(SpectraRead[grepl(ExpGroup, Condition), Condition])))]
        if (dir.exists(paste0(getwd(), "/", ExpGroup, "_vs_", CtlGroup, "_0hrOutput")) == TRUE) {
          unlink(paste0(getwd(), "/", ExpGroup, "_vs_", CtlGroup, "_0hrOutput"), recursive = TRUE)
        }
        dir.create(paste0(getwd(), "/", ExpGroup, "_vs_", CtlGroup, "_0hrOutput"), showWarnings = TRUE)
      ...
      message("Filtering Proteins")
      {
        filtering_data <- SpectraRead[,.(N_Samples = .N, Min_Precursors = min(N_precursors_L), N_conditions = data.table::uniqueN(Condition)), ProteinGroup]
      ...
      message("Performing Median Normalisation")
      {
        SpectraRead[, Log2LFQ_Norm := Log2LFQ - median(Log2LFQ, na.rm = TRUE) + median(SpectraRead$Log2LFQ, na.rm = TRUE), by = Sample]
        SpectraRead[, Colour := data.table::fifelse(Condition == CtlGroup, RColorBrewer::brewer.pal(3, "Set1")[2], RColorBrewer::brewer.pal(3, "Set1")[1])]

        Normalisation_Plot <- ggplot2::ggplot(data.table::melt.data.table(SpectraRead, measure.vars = c("Log2LFQ", "Log2LFQ_Norm")),
                                              ggplot2::aes(x = Cluster, fill = Cluster, y = value, group = Sample))+
          ggplot2::facet_wrap("variable", labeller = ggplot2::labeller(variable = c("Log2LFQ" = "Pre", "Log2LFQ_Norm" = "Post")))+
          ggplot2::geom_boxplot(outliers = FALSE) + ggplot2::ggtitle("Normalisation") +
          ggplot2::scale_fill_manual(values = Proteopedia::NiceColourPalette, guide = "none") + ggplot2::labs(x = NULL, y = "Log2LFQ")
      }
      message("Imputing NA Values")
      {
        SpectraRead <- SpectraRead[,.(ProteinGroup, Gene, Sample, Log2LFQ_Norm)] |>
      ...
      message("Fitting to Linear Model & Exporting Data")
      {
        ...
        Mean_Var_Data <- data.table::data.table(efit$genes, "Mean" = efit$Amean, "Variance" = sqrt(efit$sigma))
        Mean_Var_Data[, Data := data.table::fifelse(ProteinGroup %in% imputedNAs$ProteinGroup, "Imputed", "Measured")]

        Mean_Var_TrendPlot <- Mean_Var_Data |> ggplot2::ggplot(ggplot2::aes(x = Mean, y = Variance, colour = Data)) +
          ggplot2::geom_point() + ggplot2::scale_colour_manual(values = c("Measured" = "black", "Imputed" = "magenta3")) + ggplot2::labs(x = "Mean Log2LFQ", y = "Variance", colour = NULL)

        LimmaOutput <- limma::topTable(efit, coef=1, adjust.method = "BH", n=Inf) |>  data.table::data.table() |> data.table::setnames("logFC", "Log2FC")
        LimmaOutput <- LimmaOutput[order(abs(LimmaOutput$Log2FC), decreasing = TRUE),] |> data.table::merge.data.table(ProteinInfo, all.x = T)
        LimmaOutput[, Significance := data.table::fifelse(P.Value < 0.05 & Log2FC < 0, "Sig. Decrease", data.table::fifelse(P.Value < 0.05 & Log2FC > 0, "Sig. Increase", "None"))]
        LimmaOutput <- data.table::merge.data.table(LimmaOutput, LimmaInput, by = "ProteinGroup", all.x = TRUE)
      ...
      {
        pdf("ProteinQC_Limma_Plot.pdf", width = 18, height = 20)
        print(patchwork::free(prePCA12_plot, type = "label") + patchwork::free(prePCA34_plot, type= "label") +
                Protein_Counts_Bar + patchwork::free(Upset_Plot, type = "label") +
                patchwork::free(Normalisation_Plot) + patchwork::free(ImputedDensity) + patchwork::free(Mean_Var_TrendPlot, type = "label") + #free(all_volcano, type = "label") +
                patchwork::plot_layout(design = "AABBCCCC\nDDEEFFFF\nGGGGGGGG") + patchwork::plot_annotation(tag_levels = "A"))
        Proteopedia::Reset_Dev()
      }
      message("Exporting Analysis Parameters")
      Analysis_Parameters <- data.table::data.table("Experimental_Condition" = paste0(ExpGroup), "Control_Condition" = paste0(CtlGroup),
                                        "Min_Precursors" = paste0(MinPrecursors), "Imputation_Q-value" = ImputationQ,
                                        "Imputation_Sigma" = ImputationSigma)
      data.table::fwrite(Analysis_Parameters, "Analysis_Parameters.csv")
    }
    ...
    message(paste0(ExpGroup, " vs. ", CtlGroup, ": Comparing 0hr & Modelled Abundances"))
    {
      setwd(InputDirectory)
      pdf("AbundanceComparisonPlot.pdf", width = 16, height = 12)
      print(data.table::fread(paste0(getwd(), "/", ExpGroup, "_vs_", CtlGroup, "_0hrOutput/Limma_Output.csv"))[, .(ProteinGroup, Isoforms, Log2FC)][, Measure := "0hr"] |>
              rbind(Proteopedia::Separate_Isoforms(data.table::fread(paste0(getwd(), "/", ExpGroup, "_vs_", CtlGroup, "_Output/AbundanceData.csv")))[, .(ProteinGroup, Isoforms, Log2FC)][, Measure := "Modelled"]) |>
              ...
              Proteopedia::Add_XYLine() + Proteopedia::Add_Rsq() + ggplot2::labs(x = expression("Log"[2]~"FC in Protein Abundance (at 0 hr)"),
                                                                                 y = expression("Log"[2]~"FC in Protein Abundance (Modelled)")))
      Proteopedia::Reset_Dev()
    }
...
Process_TPP_DIANN <- function(InputDirectory, CtlGroup, SILAC, ProteotypicFiltering = F){
  ...
  if(SILAC == F){
    ...
    message("Compiling & Exporting Data")
    {
      ...
      proteotypic_counts <- PrecursorData[, .(N_precursors_proteotypic = sum(Proteotypic)), .(ProteinGroup, Sample)]
      ProteinAnnotations <- unique(PrecursorData[, .(ProteinGroup, Sample, Condition, Replicate, ProteinDescription, Gene)])
      ProteinData <- Reduce(Proteopedia::Merge_PrecursorData, list(LFQ, tot_intensities, PrecursorCounts, proteotypic_counts, ProteinAnnotations))
      data.table::fwrite(ProteinData, "LF_DIANN_Output.csv.gz")
    }
    message("Plotting Intensities")
    {
    ...
    message("Compiling & Exporting Data")
    {
      ...
      ProteinAnnotations <- unique(PrecursorData[, .(ProteinGroup, Run, Sample, Condition, Drug, Temp, Replicate, ProteinDescription, Gene)])
      ProteinData <- Reduce(Proteopedia::Merge_PrecursorData, list(LFQ_T, LFQ_H, LFQ_L, Intensity_T, Intensity_H, Intensity_L, Counts_T, Counts_H, Counts_L, ProteinAnnotations))
      ProteinData[, LFQ_Ratio := LFQ_H/LFQ_L]
      data.table::fwrite(ProteinData, "SILAC_DIANN_Output.csv.gz")
    }
...
Map_LF_Proteins <- function(InputFile, SubsetColour = "red"){
  ...
  message("Summarising Chromosome-Based Data")
  {
    ...
    ChrGroupingData <- LimmaData |> dplyr::group_by(Significance, Chromosome) |> dplyr::summarise(N = dplyr::n()) |>
      merge(LimmaData |> dplyr::group_by(Chromosome) |> dplyr::summarise(Total_N = dplyr::n())) |>
      dplyr::mutate(Prop = N/Total_N, Significance = ifelse(grepl("Sig.", Significance), Significance, "None"))
...
Map_StaticSILAC_Proteins <- function(InputDirectory, SubsetColour = "red"){
  ...
  for(Mapping in c("Total_Analysis", "Ratio_Analysis", "Heavy_Channel_Analysis", "Light_Channel_Analysis")){
    ...
    message("Calculating Biochemical Parameters")
    {
      for(ColIndex in c(which(colnames(LimmaData) == "Length"), which(colnames(LimmaData) == "Length"):ncol(LimmaData))){
        if(is.numeric(LimmaData[, get(colnames(LimmaData)[ColIndex])]) & !grepl("N_", colnames(LimmaData)[ColIndex])){
          message(paste0("Analysing ", colnames(LimmaData)[ColIndex]), " Trend")
...
Run_GSEA <- function(InputFile, OutputName = "GSEA_Output"){
  ...
  if(!is.null(InputData$Parameter)){
    for(Parameter in unique(InputData$Parameter)){
      ...
      {
        gseaBP <- clusterProfiler::gseGO(geneList= GSEAData, ont = "BP", keyType = "ENTREZID", minGSSize = 3, maxGSSize = 800, verbose = T,
                                         OrgDb = org.Hs.eg.db::org.Hs.eg.db, pAdjustMethod = "BH", pvalueCutoff = 1)
        gseaMF <- clusterProfiler::gseGO(geneList= GSEAData, ont = "MF", keyType = "ENTREZID", minGSSize = 3, maxGSSize = 800, verbose = T,
                                         OrgDb = org.Hs.eg.db::org.Hs.eg.db, pAdjustMethod = "BH", pvalueCutoff = 1)
        gseaCC <- clusterProfiler::gseGO(geneList= GSEAData, ont = "CC", keyType = "ENTREZID", minGSSize = 3, maxGSSize = 800, verbose = T,
                                         OrgDb = org.Hs.eg.db::org.Hs.eg.db, pAdjustMethod = "BH", pvalueCutoff = 1)
        gseaKEGG <- clusterProfiler::gseKEGG(geneList = GSEAData, organism = "hsa", keyType = "ncbi-geneid", verbose = T, pvalueCutoff = 0.25,
                                             pAdjustMethod = "BH")
        gseaMKEGG <- clusterProfiler::gseMKEGG(geneList = GSEAData, organism = "hsa", keyType  = "ncbi-geneid", verbose = T, pvalueCutoff = 0.25,
                                               pAdjustMethod = "BH")
        gseaReactome <- ReactomePA::gsePathway(geneList = GSEAData, organism = "human", pvalueCutoff = 1, pAdjustMethod = "BH")
        gseaDisease <- DOSE::gseDO(geneList = GSEAData, organism = "human", ont = "HDO", pvalueCutoff = 1, pAdjustMethod = "BH")
        gseaPhenotype <- DOSE::gseDO(geneList = GSEAData, organism = "human", ont = "HPO", pvalueCutoff = 1, pAdjustMethod = "BH")
        gseaDisGeNET <- DOSE::gseDGN(geneList = GSEAData, pvalueCutoff = 1, pAdjustMethod = "BH")
        gseaNCG <- DOSE::gseNCG(geneList = GSEAData, organism = "human", pvalueCutoff = 1)
        gseaHallmark <- clusterProfiler::GSEA(geneList, TERM2GENE = msigdbr::msigdbr(db_species = "HS", collection = "H"))
      }
    ...
    message("Performing Enrichment Analyses")
    {
      ...
      gseaNCG <- DOSE::gseNCG(geneList = GSEAData, organism = "human", pvalueCutoff = 1)
      gseaHallmark <- clusterProfiler::GSEA(geneList = GSEAData, TERM2GENE = msigdbr::msigdbr(species = "Homo sapiens", collection = "H") |> dplyr::select(gs_name, ncbi_gene))
    }
    ...
    message("Formatting Data For HTML Plots")
    {
      ...
      #                                                                  Proteopedia::Order_Terms(gseaDisease), Proteopedia::Order_Terms(gseaPhenotype),
      #                                                                  Proteopedia::Order_Terms(gseaDisGeNET), Proteopedia::Order_Terms(gseaNCG))))]
      suppressWarnings(MergedGSEA[, TermIndex := 1:nrow(MergedGSEA)])
      MergedGSEA[, NegLog10PAdj := -log10(p.adjust)]
      MergedGSEA[, SciPVal := formatC(p.adjust, format = "e", digits = 2)]
    }
...
Run_ORA <- function(SearchProteins, Proteome, OutputDirectory, OutputName = "ORA_Output"){
  ...
  message("Plotting & Compiling Analyses")
  {
    MergedORA <- data.table::rbindlist(list(Proteopedia::Process_ORAOutput(oraBP, "Biological_Process"), Proteopedia::Process_ORAOutput(oraMF, "Molecular_Function"),
                                            ...
                                            Proteopedia::Process_ORAOutput(oraHallmark, "Hallmark")), fill = TRUE)
...
#' @export
Run_LF_CompleteAnalysis <- function(InputDirectory, ExpGroup, CtlGroup, ProcessDIANN = F, DIANNVersion = 2.2, ProteotypicFiltering = F, MinPrecursors = 2,
...
#' @export
Run_StaticSILAC_CompleteAnalysis <- function(InputDirectory, ExpGroup, CtlGroup, ProcessDIANN = F, DIANNVersion = 2.2, ProteotypicFiltering = F, MinPrecursors = 2,
                                             ImputationQ = 0.01, ImputationSigma = 1, SubsetColour = "red", FunctionalAnalysis = F){
  start.time <- Sys.time()
  if(ProcessDIANN == T){Proteopedia::Process_StaticSILAC_DIANN(InputDirectory, CtlGroup, DIANNVersion = 2.2, ProteotypicFiltering = ProteotypicFiltering)}
  Proteopedia::Analyse_StaticSILAC_Proteins(InputDirectory, ExpGroup, CtlGroup, MinPrecursors = MinPrecursors,
                                            ImputationQ = ImputationQ, ImputationSigma = ImputationSigma, DIANNVersion = 2.2)
...
#' @export
Run_TimecourseSILAC_CompleteAnalysis <- function(InputDirectory, CtlGroup, ExpGroups, ProcessDIANN = F, ProteotypicFiltering = F, GenerateDataPlots = F,
